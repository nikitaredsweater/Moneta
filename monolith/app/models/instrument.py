...
from sqlalchemy import String, ForeignKey, Enum as PgEnum, Double, Date, Text
...
class Instrument(Base, BaseEntity):
    ...
    instrument_status: Mapped[InstrumentStatus] = mapped_column(
        PgEnum(InstrumentStatus, name="instrument_status"), nullable=False, default=InstrumentStatus.DRAFT
    )

    maturity_status: Mapped[MaturityStatus] = mapped_column(
        PgEnum(MaturityStatus, name="maturity_status"), nullable=False, default=MaturityStatus.NOT_TRADING
    )

    issuer_id: Mapped[str] = mapped_column(ForeignKey('companies.id'), nullable=False)
