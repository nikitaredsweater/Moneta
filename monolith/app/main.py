...
# Parse CORS allowed origins from environment variable (comma-separated)
# Default to localhost and internal IPs if not set
CORS_ORIGINS_ENV = os.getenv(
    'CORS_ALLOWED_ORIGINS',
    'http://localhost:3000,http://127.0.0.1:3000,http://100.64.0.8:29978',
)
CORS_ALLOWED_ORIGINS = [
    origin.strip() for origin in CORS_ORIGINS_ENV.split(',') if origin.strip()
]
logger.info('[SYSTEM] CORS allowed origins: %s', CORS_ALLOWED_ORIGINS)
